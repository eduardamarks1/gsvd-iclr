...
export const STRINGS = {
  en: {
    ...
    "hero.sub": "Most methods compare two datasets with a single number. We compare them with a geometry — and read off one interpretable angle <span class='eq-inline'>θ(z)</span> for <strong>every single sample</strong>.",
    "hero.authorsLabel": "",
    ...
    "problem.p1": "Ask how similar two datasets are and the standard tools — CKA, SVCCA, MMD, FID — hand back one scalar per pair. That scalar is a real summary, and it is often the right thing to report. But it cannot tell you <strong>which</strong> directions of the feature space carry the agreement, <strong>which</strong> structure belongs to only one of the datasets, or <strong>which</strong> individual samples are driving the number.",
    "problem.p2": "That matters the moment you want to act on the comparison. In transfer learning you do not just need to know that a source domain is similar to a target — you need to know which parts of it are worth carrying over, and which will hurt. A scalar cannot answer that by construction.",
    ...
    "problem.yes": "yes",
    "problem.no": "no",
    "problem.note": "The two views are complementary, not competing. We report CKA alongside θ throughout — it is a reference point, not a baseline we try to beat.",
    ...
    "idea.p2": "Each side answers with a coefficient vector — <span class='eq-inline'>x</span> for A, <span class='eq-inline'>y</span> for B — and we take the smallest one each side can offer. The ratio of their norms is the whole score. An arctangent turns that ratio into a bounded, symmetric angle.",
    ...
    "idea.mathBody": "<p>For a sample <span class='eq-inline'>z</span> in <span class='eq-inline'>col(A) ∩ col(B)</span>, the co-span fiber is the set of all compatible coefficient pairs:</p><div class='eq'>R(z) = { (x, y) : Ax = By = z }</div><p>Choosing the minimum-norm representative on each side — that is, <span class='eq-inline'>x ⊥ Ker(A)</span> and <span class='eq-inline'>y ⊥ Ker(B)</span> — makes the two costs canonical, and the alignment angle is</p><div class='eq'>θ(z) = arctan( ‖x‖₂ / ‖y‖₂ ) ∈ [0, π/2]</div><p>Equal costs give exactly θ = π/4. The choice of arctan is what bounds the score and makes it symmetric under swapping A and B: swapping sends θ to π/2 − θ.</p>",

    "machine.eyebrow": "The machine",
    "machine.title": "The GSVD hands you the coordinate system",
    "machine.p1": "Computing that angle by brute force for every sample would be hopeless. The Generalized Singular Value Decomposition does the work once, up front: it factors both matrices against a <em>shared</em> frame.",
    "machine.p2": "<span class='eq-inline'>H</span> is the shared ambient reference frame. The diagonal factors <span class='eq-inline'>C</span> and <span class='eq-inline'>S</span> say how strongly each direction of that frame belongs to A versus to B — and because <span class='eq-inline'>CᵀC + SᵀS = I</span>, each direction carries exactly one cosine/sine pair. That pair <em>is</em> an angle. The decomposition does not approximate the angle; it contains it.",
    ...
    "machine.blocksCaption": "C and S are padded with identity and zero blocks. The first block (C = I, S = 0) holds directions only A can produce; the last (C = 0, S = I) only B. Everything interesting lives in the middle block, where both cosine and sine are strictly positive — and where the angle sweeps continuously from 0° to 90°.",
    "machine.sliderTitle": "Walk the shared frame",
    "machine.sliderHint": "Every column of H is a direction with its own angle. Drag through them: the frame starts at the most A-like direction and ends at the most B-like one, passing through the genuinely shared structure in the middle.",
    "machine.dirLabel": "Direction",
    "machine.of": "of",
    "machine.extremesNote": "The endpoints are not found by search. The paper proves the maximizer and minimizer of θ are specific columns of H — <span class='eq-inline'>h₍ᵣ₊ₖ₎</span> and <span class='eq-inline'>h₍ᵣ₊₁₎</span> — so the same decomposition that scores samples also hands you the extreme directions in closed form.",
    "machine.mathSummary": "See the closed form",
    "machine.mathBody": "<p>The GSVD of a matrix pair sharing a row dimension gives</p><div class='eq'>A = H C U,&nbsp;&nbsp; B = H S V,&nbsp;&nbsp; CᵀC + SᵀS = I</div><p>with U and V orthogonal. Project a sample into the shared frame with <span class='eq-inline'>c(z) = H†z</span>, then the two representation costs are read off the diagonal factors:</p><div class='eq'>θ(z) = arctan( ‖C† c(z)‖₂ / ‖S† c(z)‖₂ )</div><p>This is the paper's first theorem: the GSVD-frame expression coincides exactly with the minimum-norm definition. Computationally the decomposition costs O(d³) once, and every sample afterwards costs a matrix–vector product — which is precisely why the demo below can score your drawing in real time.</p>",

    "play.eyebrow": "Playground",
    "play.title": "Draw a digit. Watch the angle move.",
    "play.p1": "Everything below runs on the real decomposition. The GSVD was computed offline with the <code>gsvdlib</code> pipeline and the resulting frame shipped to your browser, which evaluates the exact closed form as you draw — reproducing the library's own angles to within 0.02°.",
    ...
    "play.and": "and",
    "play.overlapNote": "Overlap in the middle is not a failure — it is the measurement. Samples near 45° are the ones both subspaces explain equally well, and the histogram is telling you exactly which ones they are.",
    ...
    "results.p1": "Thresholding θ at 45° gives a classifier — not a competitive one, and not the point of the work, but an honest readout of how separable the two subspaces are. Across MNIST digit pairs, accuracy falls exactly as the pair's geometric overlap rises.",
    "results.p2": "The pair 4 vs 9 is the interesting one. It has by far the highest CKA of the four, its θ histograms overlap most visibly around 45°, and it is the hardest to separate. Three independent views agreeing is what you want from a diagnostic.",
    "results.tableTitle": "MNIST, published results",
    "results.scatterTitle": "Accuracy against geometric overlap",
    "results.scatterCaption": "Each point is a digit pair. As the two class subspaces overlap more (higher CKA), the angle separates them less. The diagnostic degrades gracefully and for a legible reason.",
    ...
    "geom.p1": "θ was derived from pure linear algebra — norms of coefficient vectors. It turns out to carry a statistical meaning for free. Reading inverse squared costs as evidence gives a posterior over the two datasets:",
    "geom.p2": "Under this model, the Fisher–Rao distance between the posteriors of two samples is exactly <span class='eq-inline'>2|θ(z) − θ(z′)|</span>. The angle is not merely <em>like</em> a distance in the space of beliefs — up to a factor of two, it <em>is</em> that distance.",
    "geom.p3": "Lift this from samples to whole histograms and a second result appears: the Fisher–Rao distance between the two class-conditional θ-distributions decreases monotonically with the expected posterior standard deviation. Bins near 45° are where the posterior is closest to a coin flip, so they dominate. This is the formal version of what your eye does when it looks at the overlap of two histograms.",
    "geom.takeaway": "Mass near 45° is ambiguity, measured.",
    ...
    "next.title": "From pixels to learned spaces — and to voice",
    "next.p1": "The experiments here use raw pixels on purpose: every direction of the shared frame can be drawn as an image, so the geometry is inspectable rather than asserted. But nothing in the method is about images. A and B only need to be two matrices in a common ambient space — which is exactly what the hidden layers of a modern encoder hand you.",
    ...
    "next.q2": "The GSVD is inherently binary. Three or more domains need either aggregated pairwise angles or a genuinely multiway construction.",
    ...
    "next.q4Title": "Speech representations",
    "next.q4": "This is the direction I am most curious about. Inside a speech encoder, which directions are shared between two speakers, and which carry the identity of just one? Between two prosodic styles? Between two languages in a multilingual TTS model? Each of those is a pair of matrices in a common space — the exact shape the method expects.",
    "next.personalTitle": "A personal note",
    "next.personal": "I am Eduarda. I work as an AI researcher on Brazilian Portuguese speech synthesis, and I am heading into a master's degree where I want to bring this geometry together with voice. If you work on speech, representation learning, or just enjoy linear algebra doing honest work, I would genuinely like to hear what you would try first — including which of the questions above you think is a dead end.",
  ...
  pt: {
    ...
    "hero.sub": "A maioria dos métodos compara dois conjuntos de dados com um único número. Nós os comparamos por uma geometria — e lemos um ângulo interpretável <span class='eq-inline'>θ(z)</span> para <strong>cada amostra</strong>.",
    "hero.authorsLabel": "",
    ...
    "problem.p1": "Pergunte o quão parecidos são dois datasets e as ferramentas usuais — CKA, SVCCA, MMD, FID — devolvem um escalar por par. Esse escalar é um resumo legítimo, e muitas vezes é exatamente o que se quer reportar. Mas ele não diz <strong>quais</strong> direções do espaço de atributos carregam a semelhança, <strong>qual</strong> estrutura pertence a apenas um dos datasets, nem <strong>quais</strong> amostras individuais estão puxando o número.",
    "problem.p2": "Isso passa a importar no momento em que você quer <em>agir</em> a partir da comparação. Em transfer learning não basta saber que o domínio de origem é parecido com o de destino — é preciso saber quais partes vale a pena levar, e quais vão atrapalhar. Um escalar não responde isso por construção.",
    ...
    "problem.yes": "sim",
    "problem.no": "não",
    "problem.note": "As duas visões são complementares, não concorrentes. Reportamos CKA ao lado de θ o tempo todo — como ponto de referência, não como baseline a ser batido.",
    ...
    "idea.p2": "Cada lado responde com um vetor de coeficientes — <span class='eq-inline'>x</span> para A, <span class='eq-inline'>y</span> para B — e tomamos o menor que cada lado consegue oferecer. A razão entre as normas é o escore inteiro. Uma arcotangente transforma essa razão num ângulo limitado e simétrico.",
    "idea.p3": "Não são necessárias correspondências entre amostras, nem um mapa invertível entre domínios, nem treinamento. A relação <span class='eq-inline'>Ax = By = z</span> é o primitivo inteiro.",
    ...
    "idea.mathBody": "<p>Para uma amostra <span class='eq-inline'>z</span> em <span class='eq-inline'>col(A) ∩ col(B)</span>, a fibra co-span é o conjunto de todos os pares de coeficientes compatíveis:</p><div class='eq'>R(z) = { (x, y) : Ax = By = z }</div><p>Escolher o representante de norma mínima de cada lado — isto é, <span class='eq-inline'>x ⊥ Ker(A)</span> e <span class='eq-inline'>y ⊥ Ker(B)</span> — torna os dois custos canônicos, e o ângulo de alinhamento é</p><div class='eq'>θ(z) = arctan( ‖x‖₂ / ‖y‖₂ ) ∈ [0, π/2]</div><p>Custos iguais dão exatamente θ = π/4. A escolha da arctan é o que limita o escore e o torna simétrico ao trocar A e B: a troca leva θ em π/2 − θ.</p>",

    "machine.eyebrow": "A máquina",
    "machine.title": "A GSVD entrega o sistema de coordenadas",
    "machine.p1": "Calcular esse ângulo por força bruta para cada amostra seria inviável. A Decomposição em Valores Singulares Generalizada faz o trabalho uma vez, de antemão: ela fatora as duas matrizes contra um sistema de referência <em>compartilhado</em>.",
    "machine.p2": "<span class='eq-inline'>H</span> é o referencial ambiente compartilhado. Os fatores diagonais <span class='eq-inline'>C</span> e <span class='eq-inline'>S</span> dizem o quanto cada direção desse referencial pertence a A ou a B — e como <span class='eq-inline'>CᵀC + SᵀS = I</span>, cada direção carrega exatamente um par cosseno/seno. Esse par <em>é</em> um ângulo. A decomposição não aproxima o ângulo; ela o contém.",
    "machine.blocksTitle": "A estrutura de blocos é a história toda",
    "machine.blockA": "só A",
    "machine.blockShared": "compartilhado",
    "machine.blockB": "só B",
    "machine.blocksCaption": "C e S são preenchidos com blocos identidade e zero. O primeiro bloco (C = I, S = 0) guarda direções que só A produz; o último (C = 0, S = I), só B. Tudo o que interessa está no bloco do meio, onde cosseno e seno são estritamente positivos — e onde o ângulo varre continuamente de 0° a 90°.",
    "machine.sliderTitle": "Percorra o referencial compartilhado",
    "machine.sliderHint": "Cada coluna de H é uma direção com seu próprio ângulo. Percorra-as: o referencial começa na direção mais parecida com A e termina na mais parecida com B, passando pela estrutura genuinamente compartilhada no meio.",
    "machine.dirLabel": "Direção",
    "machine.of": "de",
    "machine.extremesNote": "Os extremos não são encontrados por busca. O artigo prova que o maximizador e o minimizador de θ são colunas específicas de H — <span class='eq-inline'>h₍ᵣ₊ₖ₎</span> e <span class='eq-inline'>h₍ᵣ₊₁₎</span> — de modo que a mesma decomposição que pontua amostras também entrega as direções extremas em forma fechada.",
    "machine.mathSummary": "Ver a forma fechada",
    "machine.mathBody": "<p>A GSVD de um par de matrizes que compartilham a dimensão de linhas dá</p><div class='eq'>A = H C U,&nbsp;&nbsp; B = H S V,&nbsp;&nbsp; CᵀC + SᵀS = I</div><p>com U e V ortogonais. Projetando uma amostra no referencial compartilhado com <span class='eq-inline'>c(z) = H†z</span>, os dois custos de representação são lidos diretamente dos fatores diagonais:</p><div class='eq'>θ(z) = arctan( ‖C† c(z)‖₂ / ‖S† c(z)‖₂ )</div><p>Esse é o primeiro teorema do artigo: a expressão no referencial da GSVD coincide exatamente com a definição por norma mínima. Computacionalmente, a decomposição custa O(d³) uma vez, e cada amostra depois custa um produto matriz–vetor — que é precisamente o motivo de o demo abaixo conseguir pontuar seu desenho em tempo real.</p>",

    "play.eyebrow": "Interativo",
    "play.title": "Desenhe um dígito. Veja o ângulo se mover.",
    "play.p1": "Tudo abaixo roda sobre a decomposição real. A GSVD foi calculada offline com o pipeline da <code>gsvdlib</code> e o referencial resultante foi enviado ao seu navegador, que avalia a forma fechada exata enquanto você desenha — reproduzindo os ângulos da própria biblioteca dentro de 0,02°.",
    ...
    "play.and": "e",
    "play.overlapNote": "A sobreposição no meio não é uma falha — é a medição. Amostras perto de 45° são as que os dois subespaços explicam igualmente bem, e o histograma está dizendo exatamente quais são.",
    ...
    "results.p1": "Cortar θ em 45° dá um classificador — não competitivo, e não é esse o ponto do trabalho, mas é uma leitura honesta de quão separáveis são os dois subespaços. Nos pares de dígitos do MNIST, a acurácia cai exatamente à medida que a sobreposição geométrica do par aumenta.",
    "results.p2": "O par 4 vs 9 é o interessante. Tem de longe o maior CKA dos quatro, seus histogramas de θ se sobrepõem de forma bem visível em torno de 45°, e é o mais difícil de separar. Três visões independentes concordando é o que se espera de um diagnóstico.",
    "results.tableTitle": "MNIST, resultados publicados",
    "results.scatterTitle": "Acurácia contra sobreposição geométrica",
    "results.scatterCaption": "Cada ponto é um par de dígitos. Quanto mais os dois subespaços de classe se sobrepõem (CKA maior), menos o ângulo os separa. O diagnóstico degrada suavemente e por um motivo legível.",
    "results.colPair": "Par",
    ...
    "geom.p1": "θ foi derivado de álgebra linear pura — normas de vetores de coeficientes. Acontece que ele carrega um significado estatístico de graça. Ler custos quadráticos inversos como evidência dá uma posteriori sobre os dois datasets:",
    "geom.p2": "Sob esse modelo, a distância de Fisher–Rao entre as posterioris de duas amostras é exatamente <span class='eq-inline'>2|θ(z) − θ(z′)|</span>. O ângulo não é apenas <em>parecido</em> com uma distância no espaço de crenças — a menos de um fator dois, ele <em>é</em> essa distância.",
    "geom.p3": "Suba isso de amostras para histogramas inteiros e um segundo resultado aparece: a distância de Fisher–Rao entre as duas distribuições condicionais de θ decresce monotonicamente com o desvio-padrão esperado da posteriori. As faixas perto de 45° são onde a posteriori mais se aproxima de um cara-ou-coroa, então dominam. Essa é a versão formal do que seu olho faz ao olhar a sobreposição de dois histogramas.",
    "geom.takeaway": "Massa perto de 45° é ambiguidade, medida.",
    ...
    "next.title": "De pixels a espaços aprendidos — e à voz",
    "next.p1": "Os experimentos aqui usam pixels brutos de propósito: cada direção do referencial compartilhado pode ser desenhada como imagem, então a geometria é inspecionável em vez de afirmada. Mas nada no método é sobre imagens. A e B só precisam ser duas matrizes num espaço ambiente comum — que é exatamente o que as camadas ocultas de um encoder moderno entregam.",
    ...
    "next.q2": "A GSVD é inerentemente binária. Três ou mais domínios exigem ou a agregação de ângulos par a par, ou uma construção genuinamente multivia.",
    ...
    "next.q4Title": "Representações de fala",
    "next.q4": "Essa é a direção que mais me interessa. Dentro de um encoder de fala, quais direções são compartilhadas entre dois falantes, e quais carregam a identidade de apenas um? Entre dois estilos prosódicos? Entre dois idiomas num modelo de TTS multilíngue? Cada um desses casos é um par de matrizes num espaço comum — exatamente o formato que o método espera.",
    "next.personalTitle": "Uma nota pessoal",
    "next.personal": "Eu sou a Eduarda. Trabalho como pesquisadora de IA em síntese de voz em português brasileiro, e estou indo para um mestrado no qual quero unir esta geometria com voz. Se você trabalha com fala, com aprendizado de representações, ou simplesmente gosta de ver álgebra linear fazendo trabalho honesto, eu gostaria muito de saber o que você tentaria primeiro — inclusive qual das perguntas acima você acha que é beco sem saída.",
